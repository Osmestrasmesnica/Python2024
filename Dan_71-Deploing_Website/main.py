...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        new_comment = Comment(
            ...
            # author_id=current_user.id, # --> pošto sam postavio relationship onda ne mora ovo da se piše
            # post_id=requested_post.id, # --> pošto sam postavio relationship onda ne mora ovo da se piše
        )
    ...
    return render_template("post.html", form=form, post=requested_post, godina=godina, logged_in=current_user.is_authenticated, current_user=current_user, posted_time=the_time)
...
# The function for deleting comments if you are author
@app.route("/delete/comment/<int:id>/<int:post_id>")
@only_commenter
def delete_comment(post_id, id):
    post_to_delete = db.get_or_404(Comment, id)
    db.session.delete(post_to_delete)
    db.session.commit()
    return redirect(url_for('show_post', post_id=post_id, logged_in=current_user.is_authenticated))
...
#! PROVERITI KADA BUDES NEKADA IMAO VREMENA ZASTO OVO NE RADI LEPO KADA OBRISES "posts.db"
#! IZBACUJE TI ERROR 404
#! SKONTAO SI DA TREBA DA SE UBACI USER PREKO DB BROWSERa(ti si morao nekoliko njih da ubacis) DA BI PRORADILO